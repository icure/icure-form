...
- Status: accepted
- Date: 2026-09-07
- Related: [PRD: Hide empty fields in read-only rendering](../../plans/prd-hide-empty-fields.md), CONTEXT.md "Read-only review"
...
- **Resolution pre-pass.** An async walk over the whole definition and container tree (mirroring the card renderer's `flattenWithVisibility`) computing a survival set consumed by both the render pass and the tab bar. Preserves lazy DOM instantiation but evaluates every Section's formulas and values on each render. Rejected: it violates point 1.
...
- Hosts using `hideEmptyFields` with `form:tab` will see tabs for Sections that contain nothing; the PRD records this as accepted behaviour and the glossary entry for `hideEmptyFields` states it.
