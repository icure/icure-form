...
**Suggestion** — An item proposed to the clinician in response to a search: typically a code from a codification (ICD-10, ICPC-2, a custom entity list), carrying per-language Labels, an optional per-language Insertion, and the search terms it answers. Suggestions are produced by a Suggestion provider and consumed by a suggestion surface.

**Insertion** — What selecting a Suggestion produces, as opposed to what it displayed: the text that replaces the typed words in a Text field, or the value a Dropdown stores. Per-language, and optional — a Suggestion that omits it inserts its Label. Distinct from the Label because the two serve different readers: a palette row may read *Predominantly allergic asthma* while the record receives *J45.0*.

**Wildcard language** — The key `'*'` in a Suggestion's Label or Insertion map, matching any language. An exact language key always wins over it. It is how a provider serves content that has no translation to give — a person's name, a bare code number — without repeating it under every language.
