...
/** Runs one formula as the sole computed field of a form, and returns what got stored. */
const storedValueOf = async (formula: string, label: string): Promise<{ [language: string]: PrimitiveType } | undefined> => {
	...
	return Object.values(values)[0]?.[0]?.value?.content
}
