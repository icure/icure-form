...
// *before* the demo-app module loads, because each theme's index.ts calls
// customElements.define() at import time and Custom Elements can only be
// registered once per tag name. Switching themes therefore requires a soft
// reload — the picker writes a new value to localStorage and calls
// location.reload().
...
const themeImporters: Record<DemoTheme, () => Promise<unknown>> = {
...
void (async () => {
	await themeImporters[getStoredTheme()]()
