...
describe('endOfLife services across the contact history', () => {
	...
	const rev1 = () =>
		new DecryptedContact({
			...
			created: t0,
			services: [
				new DecryptedService({ id: DELETED_SERVICE_ID, label: 'DeletedField', created: t0, modified: t0, content: { en: new DecryptedContent({ stringValue: 'to be deleted' }) } }),
				new DecryptedService({ id: OTHER_SERVICE_ID, label: 'OtherField', created: t0, modified: t0, content: { en: new DecryptedContent({ stringValue: 'other' }) } }),
			],
			subContacts: [new DecryptedSubContact({ formId: FORM_ID, services: [new ServiceLink({ serviceId: DELETED_SERVICE_ID }), new ServiceLink({ serviceId: OTHER_SERVICE_ID })] })],
		})
	...
	const rev2WithTombstone = () =>
		new DecryptedContact({
			...
			rev: '2-def',
			created: t0 + 10000,
			services: [new DecryptedService({ id: DELETED_SERVICE_ID, created: t0, modified: t0 + 10000, endOfLife: t0 + 10000 })],
			subContacts: [new DecryptedSubContact({ formId: FORM_ID, services: [new ServiceLink({ serviceId: DELETED_SERVICE_ID })] })],
		})
	...
	it('does not display a service whose endOfLife tombstone only exists in a past contact version', () => {
		...
		const rev3 = new DecryptedContact({
			id: 'contact-1',
			rev: '3-ghi',
			created: t0 + 20000,
			services: [new DecryptedService({ id: OTHER_SERVICE_ID, label: 'OtherField', created: t0, modified: t0 + 20000, content: { en: new DecryptedContent({ stringValue: 'other, edited' }) } })],
			subContacts: [new DecryptedSubContact({ formId: FORM_ID, services: [new ServiceLink({ serviceId: OTHER_SERVICE_ID })] })],
		})
	...
	it('displays a service again when it was re-created after having been tombstoned', () => {
		...
		const rev3 = new DecryptedContact({
			...
			rev: '3-ghi',
			created: t0 + 20000,
			services: [new DecryptedService({ id: DELETED_SERVICE_ID, label: 'DeletedField', created: t0, modified: t0 + 20000, content: { en: new DecryptedContent({ stringValue: 'recreated' }) } })],
			subContacts: [new DecryptedSubContact({ formId: FORM_ID, services: [new ServiceLink({ serviceId: DELETED_SERVICE_ID })] })],
		})
