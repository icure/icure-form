...
import { localized } from './suggestions'
 ...
 * so a promotion applies within its own group only. `code`, `terms`, `matched`, `insertion` and `children` survive the
 * mapping; a flat `Code` input yields `{ id, label, terms: [] }`. Nothing is derived from `label` here any more: what a
 * surface displays or inserts is resolved at read time (see src/utils/suggestions.ts), so a `'*'` label and a
 * per-language `insertion` both keep their meaning all the way down.
 */
export const sortSuggestions = (codes: (Code | Suggestion)[], language: string, sortOptions?: SortOptions): Suggestion[] => {
	...
	return codes.sort(comparator).map((c) => {
		...
		return {
			id: c.id,
			label: c.label ?? { [language]: c.id },
			terms: s.terms ?? [],
			...(c.code !== undefined ? { code: c.code } : {}),
			...(s.insertion !== undefined ? { insertion: s.insertion } : {}),
...
export const filterAndSortOptionsFromFieldDefinition = (language: string, fg: Field, translationProvider: ((language: string, text: string) => string) | undefined, terms?: string[]) =>
	Promise.resolve(
		sortCodes(
			optionMapper(language, fg, translationProvider).filter((x) => (terms ?? []).map((st) => st.toLowerCase()).every((st) => (localized(x.label, language) ?? x.id).toLowerCase().includes(st))),
			language,
			fg.sortOptions,
		),
	)
